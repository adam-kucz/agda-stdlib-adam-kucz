...
fmap-id ⦃ ListFunctor ⦄ = subrel $ fun-ext go
  where go : map (𝑖𝑑 X) ~ 𝑖𝑑 (List X)
        go [] = refl []
        go (h ∷ t) = ap (h ∷_) (go t)
fmap-∘ ⦃ ListFunctor ⦄ g f = subrel $ fun-ext $ map-∘ g f
...
open import Type.Identity.Homogeneous
...
unit2 ⦃ ListMonad ⦄ = subrel $ fun-ext go
  where go : mconcat ∘ pure ~ id
        go [] = Het.refl []
        go (h ∷ t) = ap (h ∷_) (go t)
mon-naturality ⦃ ListMonad ⦄ f = subrel $ fun-ext go
